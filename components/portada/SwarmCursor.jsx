...
/**
 * SwarmCursor — React Bits (variante JavaScript + CSS), sobre `ogl`.
 *
 * Un enjambre de partículas que persigue al puntero y se funde en una sola
 * masa cuando se juntan. Va solo en la portada y solo en escritorio: monta un
 * segundo contexto WebGL además del de la ficha, y eso en un teléfono no se
 * paga (ver `SwarmCursorGate`).
 *
 * Cambios sobre el original:
 *   - no envuelve al contenido: es una capa suelta que se coloca sobre la
 *     portada. Envolverlo obligaba a montar el contenido dentro de un
 *     componente cliente diferido, y el titular es el LCP;
 *   - por lo mismo, escucha el puntero en `window` y traduce a coordenadas
 *     de la capa, en vez de escuchar en su propio contenedor;
 *   - el lienzo queda por detrás del contenido (ver SwarmCursor.css).
 */
...
const SwarmCursor = ({
  ...
  useEffect(() => {
    ...
    // En `window`, no en el contenedor: la capa no recibe eventos (está en
    // pointer-events:none) y encima el enjambre tiene que seguir al ratón
    // también cuando pasa por encima de la ficha o de un botón.
